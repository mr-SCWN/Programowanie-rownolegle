...
void SumMatrixGPU(int N, int R, int K, int matrix_size, const char* mode, float* input_matrix, float* output_matrix, dim3 threadGrid, dim3 blockGrid, int shared_memoii_size, float& avg_time, float& avg_speed) {
    ...
    for (int iter = 0; iter < 5; iter++) { // zmiana na iter = 1, żeby nie obliczało kilku razy (dla nvidia nsight)
   ...
   verifyMatrices(output_matrix, hostOutput, N, R);
    ...
    avg_time = totalElapsedTime / 5.0f; // zmiana na / 1.0f , żeby nie obliczało kilku razy (dla nvidia nsight)
...
int main() {
    ...
    for (auto& val : Values) {
        ...
        float avgElapsedTime_CPU = totalElapsedTime_CPU / 5.0f; // zmiana na /1.0f , żeby nie obliczało kilku razy (dla nvidia nsight)
